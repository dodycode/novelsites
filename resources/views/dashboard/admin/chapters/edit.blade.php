	...
	<div class="row">
		...
		<div class="col-xs-12">
			<div class="box">
				...
				<form role="form" method="POST" action="{{ route('admin.chapter.storeEdit', ['id' => $chapter->id]) }}">
					{{ csrf_field() }}
					<div class="box-body">
						<div class="form-group">
							...
							<select name="id_novel" id="select1" class="form-control select2" required>
								...
								<option <?php if($chapter->id_novel == $novel->id){echo "selected";} ?> value="{{ $novel->id }}">{{ $novel->judul_novel }}</option>
								@endforeach
							</select>
						</div>
						<div class="form-group">
							<label>Chapter</label>
							<input type="text" class="form-control" name="chapter" value="{{ $chapter->chapter }}" maxlength="20" required>
						...
						<div class="form-group">
							...
							<select name="id_ft" id="select2" class="form-control select2" required>
								...
								<option <?php if($chapter->id_ft == $ft->id){echo "selected";} ?> value="{{ $ft->id }}">{{ $ft->nama_ft }}</option>
								@endforeach
							</select>
						</div>
						<div class="form-group">
							<label>Link to Chapter</label>
							<input type="url" name="url" value="{{ $chapter->url }}" class="form-control" required>
							<small>Sertakan alamat lengkap hingga http atau https nya jika menggunakan https</small>
						</div>
						<div class="form-group">
							<label>Release Date</label>
							<input type="text" id="datepicker" value="{{ $chapter->tanggal }}" class="form-control" name="tanggal">
